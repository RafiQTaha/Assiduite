...
class ApiController extends AbstractController
{
 ...
 static function traitement_d($code_admission,$promotion,$groupe,$date)
 {
  ...
  static function traitement_abcd($promotion,$date,$date1,$date2,$salle,$groupe, $em)
 {
    ...
    if ($groupe == 'empty') {
     ...
     $date1 = date('H:i:s',strtotime($date1));
        ...
        $requete = "select DISTINCT userinfo.street as adm,x_inscription_grp.nom,x_inscription_grp.prenom,x_inscription_grp.Grp_Stg,min(date_format(checkinout.CHECKTIME,'%H:%i')) as pointage,'$date' as date from userinfo inner join x_inscription_grp on userinfo.street = x_inscription_grp.code_admission
        ...
        AND CHECKTIME>='$date $date1' AND CHECKTIME<='$date2' GROUP BY userinfo.street ORDER BY x_inscription_grp.nom";
        // $requete = "select DISTINCT userinfo.street as adm,x_inscription_grp.nom,x_inscription_grp.prenom,x_inscription_grp.Grp_Stg,min(date_format(checkinout.CHECKTIME,'%H:%i')) as pointage,'$date' as date from userinfo inner join x_inscription_grp on userinfo.street = x_inscription_grp.code_admission
        // LEFT join checkinout on checkinout.USERID = userinfo.USERID
        // LEFT JOIN iseance_salle ON checkinout.sn=iseance_salle.id_pointeuse
        // WHERE x_inscription_grp.promotion='$promotion'  AND checkinout.CHECKTIME LIKE '$date%' AND iseance_salle.code_salle='$salle'
        ...
        foreach ($result as &$item) {
            ...
            if ($pointage >= $hd1 && $pointage <= $hf1) {
                $categorie = 'A';
            } elseif ($pointage >= $hd2 && $pointage <= $hf2) {
                $categorie = 'B';
            } elseif ($pointage >= $hd3 && $pointage <= $hf3) {
                $categorie = 'C';
            } elseif ($pointage >= $hd4 && $pointage <= $hf4) {
                $categorie = 'D';
            } elseif ($pointage === null) {
                $categorie = 'D';
            }
...
//  }
 ...
 static function coun_categorie($idseance)
 {
    ...
    $requete ="SELECT id_séance,
                SUM(CASE WHEN categorie = 'A' THEN 1 ELSE 0 END) AS A,
                SUM(CASE WHEN categorie = 'B' THEN 1 ELSE 0 END) AS B,
                SUM(CASE WHEN categorie = 'C' THEN 1 ELSE 0 END) AS C,
                SUM(CASE WHEN categorie = 'D' THEN 1 ELSE 0 END) AS D
 ...
 {      
   if ($type == "stage") {
    ...
    foreach ($seances as &$se){
        $scan = 0;
        if(file_exists('\\\172.20.0.54\uiass\pdf\\'.$se["code_seance"].'.pdf')==1){
            $scan=1;
        }else{
            $scan = 0;
        }
...
#[Route('/Etud_pointage', name: 'Etud_pointage')]
 ...
 {   
    if($request->isXmlHttpRequest()) {
        ...
        $requete_seance = "SELECT promotion,module,groupe,heure_debut,heure_fin,code_salle FROM `v_seance` WHERE code_seance=$seance limit 1";
        ...
        // try {
            self::pointeuse_ip($salle,'traite',$date, $date,$this->em);
            // self::pointeuse_ip($seance,$this->em);
        // } catch (\Throwable $th) {
        //     $status = 'error import';
        //     // dd($th);
        // }
        ...
        $etudiants_abcd = self::traitement_abcd($promotion,$date,$newDateD,$newDateF,$salle,$groupe,$this->em);
...
#[Route('/pointeuse_download/{idsalle}/{date_debut}/{date_fin}', name: 'pointeuse_download')]
public function pointeuse_download(Request $request, $idsalle,$date_debut, $date_fin)
{  
   
    $data = self::pointeuse_ip($idsalle,'import',$date_debut , $date_fin,$this->em);
...
#[Route('/pointeuse_att/{idsalle}/{date_debut}/{date_fin}', name: 'pointeuse_att')]
    public function pointeuse_att(Request $request, $idsalle,$date_debut, $date_fin)
    {  
        ...
        if($date_fin != 0){
            // dd("2 dates");
             $user_device = $zk->getAttendance_date($date_debut, $date_fin);
        }else{
            // dd("1 dates");
            $user_device = $zk->getAttendance($date_debut);
        }
...
static function pointeuse_insert($idsalle,$sn,$date_debut, $date_fin,$em)
{  
   
$zk = new \ZKLib("$idsalle", 4370, "udp");

 $zk_ = new \ZKLibrary("$idsalle", 4370, 'udp');
...
if ($zk->connect() == 'true') {
               ...
               $statut = "device connected";
                ...
                $sn = substr($sn, 14, -1);
                try {
                    $targetDate2 = date('Y-m-d', strtotime($date_fin . ' +1 day'));

                    if($date_fin != 0){
                        $attendance = $zk->getAttendance_date($date_debut,$targetDate2);
                    }else{

                        $attendance = $zk->getAttendance($date_debut);
                    }
                ...
                $zk->disconnect();
                $counter = 0;
                 $pointeuse_date = $em->getRepository(Checkinout::class)->findOneBy(['sn'=>$sn],['CHECKTIME'=>'DESC']);
                // && !empty($attendance

                if (count($attendance) > 0 || !empty($attendance)) {
                    ...
                    foreach ($attendance as $attItem) {
                        ...
                        $date_ = new \DateTime($attItem["timestamp"]);
                            ...
                            if ($user) {
                                ...
                                if(!$pointeuse_date || $date_ > $pointeuse_date->getCHECKTIME()){
                                    $counter++;
...
//             $zk->disableDevice();
//             $sn = substr($zk->serialNumber(), 14, -1);
//             $attendance = $zk->getAttendance($date);
...
//         $statut = "'{$idsalle}' device is connected";
//     } finally {
//         // Disconnect at the end, whether the operation succeeded or not
//         if ($zk->connect() === 'true') {
//             $zk->disconnect();
//         }
//     }
...
static function ping($ip)
{
    ...
    static function pointeuse_ip($salle,$type,$date_debut, $date_fin ,$em)
    {
        if ($type == 'traite') {
            // dd('traiter');   
            $pointeuse_status = [];
            ...
            foreach ($ip as $ips) {
                ...
                if ($ping == 'yes') {
                    ...
                    $data = self::pointeuse_insert($ips['IP'],$ips['sn'],$date_debut, $date_fin,$em);
        ...
        // dd($pointeuse_status);
            ...
            $ping =  self::ping($salle);

            if ($ping == 'yes') {

            $data = self::pointeuse_insert($salle,'sn',$date_debut, $date_fin,$em);
    ...
    // static function pointeuse_ip($seance_id, $em)
    // {  
    //     $seance = $em->getRepository(PlEmptime::class)->find($seance_id);
    //     $seanceSalle = $em->getRepository(ISeanceSalle::class)->findOneBy(['code_salle'=>$seance->getSalle()->getCode()]);
    //     $machines = $em->getRepository(Machines::class)->findBy(['sn' => $seanceSalle->getIdPointeuse()]);
    //     // dd($seanceSalle);
    ...
    //         $heure_seance = $seance->getStart();
    //         $date_seance = date_format($heure_seance, 'Y-m-d');
    ...
    //             sleep(1);
    //             $chechtime = $em->getRepository(Checkinout::class)->findBy(array('sn' => $SN), array('CHECKTIME' => 'DESC'), 1, 0);
    //             // $time = $chechtime[0]->getCHECKTIME();
    //             // dd($time);
    //             if (!empty($chechtime)) {
    //                 $time = date_format($chechtime[0]->getCHECKTIME(), 'Y-m-d H:i:s');
    //             } else {
    //                 $time = "2021-04-15 07:40:00";
    //             }
    //             foreach ($attendance as $id) {

    //                 // dd('lest in poineuse', $id[3], 'last in table', $time);
    //                 if ($id[3] > $time) {
    //                     $val = $em->getRepository(Userinfo::class)->findOneBy(array('Badgenumber' => $id[1]));
    //                     $checkinout = new Checkinout;
    //                     $checkinout->setUSERID($val->getId());
    //                     $checkinout->setCHECKTIME(new \DateTime($id[3]));
    //                     $checkinout->setCHECKTYPE($id[2]);
    //                     $checkinout->setSn($SN);
    //                     $em->persist($checkinout);
    //                     $em->flush();
    //                 }
    ...
    /**
     * @Route("/open/{salle}", name="open")
     */
    public function open(Request $request, $salle)
    {


     $sal = (int)($salle);
       @$sll = $sal;
        $file = '\\\172.20.0.54\uiass\pdf\\'.$sal.'.pdf';
        $filname = '"'.$salle.'"hello.pdf';
        // dd($file);
