...
switch ($semestre) {
    ...
    INNER JOIN v_seance       ON v_seance.code_seance=xseance.id_séance
      WHERE xseance_absences.ID_Admission='$idadm' AND semaine.annee_s='2023/2024' 
...
//    dd($mysql);  
        ...
        $mysql2="SELECT xseance_absences.ID_Admission,xseance.ID_Séance,xseance.ID_Module,semaine.nsemaine as Semaine,semaine.date_debut as DateD,
        semaine.date_fin as DateF,xseance_absences.Nom,xseance_absences.Prénom,xseance.Année_Lib 
        FROM xseance 
        
        INNER JOIN semaine ON xseance.semaine=semaine.id  
        INNER JOIN xseance_absences ON xseance.ID_Séance=xseance_absences.ID_Séance
        WHERE xseance_absences.ID_Admission='$idadm'  AND semaine.annee_s='2023/2024'   ORDER BY semaine.nsemaine limit 1";

            $sean2 =  ApiController::execute($mysql2,$this->em);
            if (empty($sean2)) {
                return new JsonResponse(['message' => 'PDF vide [aucun seance traité pour l etudiant!'], JsonResponse::HTTP_BAD_REQUEST);
            ...
            $mysql3="SELECT x_inscription_grp.code_admission,ac_etablissement.designation as etablissement,ac_formation.designation as form,ac_promotion.designation as promo,x_inscription_grp.nom,x_inscription_grp.prenom
...
$date2 = strftime("%A %d %B %G", strtotime($date)); 
   ...
   xseance.ID_Séance,v_seance.nature_des,xseance.Groupe,v_seance.salle_des AS salle, v_seance.module_des as module, 
...
public function Residanatimp(Request $request): Response
{
    ...
    public function regularisation_date(Request $request, SluggerInterface $slugger): JsonResponse
    {
        ...
        $dat= date_format($TodayDate, 'Y-m-d H:i');
            foreach($sheetData as $sheet) {
                ...
                INNER JOIN xseance  ON xseance.ID_Séance=xseance_absences.ID_Séance
                SET `Categorie`="'.$sheet[8].'",`Categorie_Enseig`="",`Obs`="'.$sheet[10].'" 
                
                WHERE xseance_absences.ID_Admission="'.$sheet[0].'" AND  xseance.Date_Séance BETWEEN "'.$sheet[6].'" AND "'.$sheet[7].'"' ;
                // dd($insert);
                // $execute =  self::execute($insert,$this->em);
                $stmt = $this->em->getConnection()->prepare($insert);
                $newstmt = $stmt->executeQuery();   
    ...
    #[Route('/affichagesalle', name: 'assiduite_assiduites_affichagesalle')]
public function affichagesalle(Request $request): Response
{
